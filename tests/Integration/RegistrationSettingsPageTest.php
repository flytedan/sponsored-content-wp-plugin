 ...
 * WordPress install - specifically the SEO-plugin warning banner added on
 * top of {@see Resolver::has_recommended_plugin()}, since that's real HTML
 * output the unit suite's Brain Monkey mocks can't usefully verify.
 */
final class RegistrationSettingsPageTest extends WP_UnitTestCase {
	...
	public function test_render_shows_the_seo_warning_when_no_supported_plugin_is_active(): void {
		...
		$page = new RegistrationSettingsPage( $this->client(), new Resolver() );
	...
	public function test_render_hides_the_seo_warning_when_a_supported_plugin_is_active(): void {
		...
		$page     = new RegistrationSettingsPage( $this->client(), $resolver );
