...
- `flytedesk/v1` REST API: `POST /posts`, `GET /posts/{id}`, `PUT /posts/{id}`, `DELETE /posts/{id}`, authenticated via a flytedesk-owned API key (`Authorization: Bearer <api key>`), entirely disconnected from WordPress's own authentication system - no WordPress user, capability, cookie, or nonce is ever involved.
...
- Registration with sponsored.flytedesk.com, gated behind an explicit "Connect to flytedesk" opt-in (see Changed below): generates a flytedesk API key - a random secret owned entirely by this plugin, not tied to any WordPress user or Application Password - and sends it in the registration request so flytedesk's platform can start using this site's API immediately once a human accepts the registration. The inbound `/flytedesk-registration-confirmation` webhook is authenticated via a separate bearer token shared during registration.
...
- Deactivating now revokes the flytedesk API key outright, cutting off access the moment the plugin is turned off; registration status/token are left intact so reactivating re-registers rather than starting over. Deleting the plugin (`uninstall.php`) goes further, additionally removing every other `flytedesk_*` option - a clean slate. Neither step touches already-published `fdhc_hosted_post` content.
...
- `seo.og.type` now defaults to `"article"` on create/update when the request omits it, since every `fdhc_hosted_post` is one.
